...
import level_selection.model_representation.DecisionTreeClassifier;

/**
 * This Class runs the {@code model_represantation.DecisionTreeClassifier} with the input features
 */
public class DecisionTreeClassifierModel implements ClassifierModel {
	...
	public Map<Integer, Double> predict(Map<Integer, Map<String, Integer>> levelFeatures) {
		...
		for (Map.Entry<Integer, Map<String, Integer>> entry : this.levelFeatures.entrySet()) {
			...
			int[] predArr = DecisionTreeClassifier.predict(featureMap.get("max_score"), featureMap.get("num_birds"),
