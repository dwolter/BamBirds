...
public class ShowSeg implements Runnable {
	...
	public static void main(String[] args)
	{
		...
		if (args.length > 1) {
			CustomLogger.warning("  USAGE: java TestVision [(<directory> | <image>)]");
			System.exit(1);
		}
		...
		if (args.length == 0) {
			...
			while (true) {
				...
				try {
					...
					CustomLogger.severe("Thread Interrupted");
				}
			}
		}
		...
		for (File filename : images) {
			...
			try {
				...
				CustomLogger.severe("ERROR: could not load image " + filename);
				System.exit(1);
			}
		...
		BufferedImage screenshot = null;
       	while (true) {
			...
			try {
				...
				CustomLogger.severe(" Thread Interrupt");
			}
		}
	}
