...
public class VisionMBR {
	...
	public VisionMBR(BufferedImage screenshot) {
		...
		if (USE_NEW_SLING_DETECTION) {
			hsv = new VisionSling(screenshot);
		}
	...
	public Rectangle findSlingshotMBR() {
		Rectangle obj;
		if (USE_NEW_SLING_DETECTION) {
			obj = hsv.findSlingshot();
			if (obj != null)
				return obj;
			CustomLogger.warning("Could not find slingshot with new method, using old VisionMBR method ...");
		}
	...
	public Integer query(Point p) {
		if ((p.x >= _nWidth) || (p.y >= _nHeight)) {
			CustomLogger.severe("pixel (" + p.x + ", " + p.y
					+ ") is out of range");
			return null;
		}
	...
	private void processScreenShot(BufferedImage screenshot) {
		...
		if ((_nHeight != 480) && (_nWidth != 840)) {
			CustomLogger.severe("ERROR: expecting 840-by-480 image, got " + _nWidth + "x" + _nHeight);
			System.exit(1);
		}
		...
		for (int y = 0; y < _nHeight; y++) {
			for (int x = 0; x < _nWidth; x++) {
				...
				_scene[y][x] = ((colour & 0x00e00000) >> 15)
						...
						| ((colour & 0x000000e0) >> 5);
				//CustomLogger.info(" x " + x + " y " + y + " 3-bit rgb " + _scene[y][x]);
			}
		}
		...
		// CustomLogger.info("...found " + _nSegments + " components");
