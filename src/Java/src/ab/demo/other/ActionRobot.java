...
public class ActionRobot {
	...
	static {
		if (proxy == null) {
			...
			CustomLogger.info("Server started on port: " + proxy.getPort());
	...
	public static void GoFromMainMenuToLevelSelection() {
		...
		while (state == GameState.MAIN_MENU) {
			...
			CustomLogger.info("Go to the Episode Menu");
		...
		while (state == GameState.EPISODE_MENU) {
			CustomLogger.info("Select the Poached Eggs Episode");
	...
	public GameState shootWithStateInfoReturned(List<Shot> csc) {
		...
		ss.shoot(proxy, csc);
		CustomLogger.info("Shooting Completed");
		GameState state = StateUtil.getGameState(proxy);
		return state;
	...
	public void shoot(List<Shot> csc) {
		...
		ss.shoot(proxy, csc);
		CustomLogger.info("Shooting Completed");
		CustomLogger.info("wait 15 seconds to ensure all objects in the scene static");
		try {
			Thread.sleep(15000);
		} catch (InterruptedException e) {
			e.printStackTrace();
	...
	public void fshoot(Shot shot) {
		...
		shots.add(shot);
		ss.shoot(proxy, shots);
		// CustomLogger.info(" tap time : " + shot.getT_tap());
		CustomLogger.info("Shooting Completed");
	...
	public static void main(String args[]) {
		...
		CustomLogger.info(" time to take 40 screenshots" + (System.currentTimeMillis() - time));
		System.exit(0);
