					...
					if(command.equalsIgnoreCase("-pshoot"))
					{
						...
						if (args.length == 2 && command.equalsIgnoreCase("-na"))
						{
							...
							if(! (args[1].equalsIgnoreCase("-showMBR") || args[1].equals("-showReal")))
							{
								...
								{
									CustomLogger.warning("wrong level number, will use the default one");
								}
						...
						else if (command.equalsIgnoreCase("-recordImg"))
						{
							...
							if(args.length < 2)
								CustomLogger.info("please specify the directory");
							else
							{
								String[] param = {args[1]};   
							...
							CustomLogger.info("Please input the correct command");
		}
		else 
			CustomLogger.info("Please input the correct command");
