...
public class ClientNaiveAgent implements Runnable {
	...
	private void checkMyScore()
	{
		...
		CustomLogger.info(" My score: ");
		int level = 1;
		for(int i: scores)
		{
			CustomLogger.info(" level " + level + "  " + i);
			if (i > 0)
				solved[level - 1] = 1;
			level ++;
		}
	...
	public void run() {	
		...
		while (true) {
			...
			if (state == GameState.WON) {
				...
				///CustomLogger.info(" loading the level " + (currentLevel + 1) );
				checkMyScore();
				currentLevel = (byte)getNextLevel();
				ar.loadLevel(currentLevel);
				//ar.loadLevel((byte)9);
				...
				CustomLogger.info("Global best score: ");
				for (int i = 0; i < scores.length ; i ++)
				{
				
					CustomLogger.info( " level " + (i+1) + ": " + scores[i]);
				}
				...
				{		
					CustomLogger.info("restart");
					ar.restartLevel();
				}
						
			} else 
				if (state == GameState.LEVEL_SELECTION) {
				CustomLogger.warning("unexpected level selection page, go to the last current level : "
								+ currentLevel);
				ar.loadLevel(currentLevel);
			} else if (state == GameState.MAIN_MENU) {
				CustomLogger.warning("unexpected main menu page, reload the level : "
								+ currentLevel);
				ar.loadLevel(currentLevel);
			} else if (state == GameState.EPISODE_MENU) {
				CustomLogger.warning("unexpected episode menu page, reload the level: "
								+ currentLevel);
				ar.loadLevel(currentLevel);
			}
			...
			if (!pigs.isEmpty()) {		
				...
				// random pick up a pig
					...
					if (prevTarget != null && distance(prevTarget, _tpt) < 10) {
						...
						CustomLogger.info("Randomly changing to " + _tpt);
					}
					...
					if (firstShot && pts.size() > 1) {
						...
						if (pts.size() == 1)
							...
							if(pts.size() == 2)
							{
								// CustomLogger.info("first shot " + firstShot);
					...
					if (releasePoint != null) {
						...
						CustomLogger.info("Release Angle: "
						...
						{
							CustomLogger.warning("No Release Point Found");
					...
					if(_sling != null)
					{
						...
						if(scale_diff < 25)
						{
							...
							if(dx < 0)
							{
								...
								ar.shootSafe(refPoint.x, refPoint.y, dx, dy, 0, tapTime, false);
								CustomLogger.info("It takes " + (System.currentTimeMillis() - timer) + " ms to take a shot");
								state = ar.checkState();
							...
							CustomLogger.warning("Scale is changed, can not execute the shot, will re-segement the image");
					}
					else
						CustomLogger.warning("no sling detected, can not execute the shot, will re-segement the image");
