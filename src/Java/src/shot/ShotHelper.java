...
public class ShotHelper {
	...
	static public void setProperties(double scaling, ABType birdType) {
		...
		highAngleBegin    = Constants.highAngleBegin[birdType.id];
		highAngleChange   = Constants.highAngleChange[birdType.id];
		highAngleVelocity = Constants.highAngleVelocity[birdType.id];
	}

	static private double fn(double[] w, double x) { return w[0] * x * x + w[1] * x + w[2]; }

	static public double fYellowVelocity(double x) { return fn(Constants.yellowBirdVelocity, x); }
	...
	static private double fLaunchVelocity(double x) {
		...
		return fn(Constants.lowAngleVelocity, x);
	}
	...
	static private double fChangeAngle(double x) {
		...
		return fn(Constants.lowAngleChange, x);
	}
	...
	/**
	 * Calculate flight time. Can be used as tap time t2 in {@link ab.demo.other.ClientActionRobot#shoot}
	 * @param theta Actual angle, <b>NOT</b> release point. <br />
	 *              To convert release point use {@link #launchToActual(double)}
