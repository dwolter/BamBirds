...
package de.uniba.sme.bambirds.debugging.proxy;
...
public class Proxy extends WebSocketServer {
    ...
    public void onOpen(WebSocket conn, ClientHandshake handshake) {
    	//CustomLogger.info(conn.getRemoteSocketAddress().toString());
    	onOpen();
    }
    ...
    public void waitForClients(int numClients) {
        ...
        while (getConnections().size() < numClients) {
            try {
                Thread.sleep(100);
            } catch (InterruptedException e) {
            }
