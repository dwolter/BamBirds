...
public class VisionMBR {
	private static final Logger log = LogManager.getLogger(VisionMBR.class);
	...
	public Rectangle findSlingshotMBR() {
		...
		if (USE_NEW_SLING_DETECTION) {
			...
			if (obj != null)
				return obj;
			log.warn("Could not find slingshot with new method, using old VisionMBR method ...");
		}
	...
	private void processScreenShot(BufferedImage screenshot) {
		...
		for (int y = 0; y < _nHeight; y++) {
			for (int x = 0; x < _nWidth; x++) {
				...
				_scene[y][x] = ((colour & 0x00e00000) >> 15)
						...
						| ((colour & 0x000000e0) >> 5);
				//log.info(" x " + x + " y " + y + " 3-bit rgb " + _scene[y][x]);
			}
		}
		...
		// log.info("...found " + _nSegments + " components");
