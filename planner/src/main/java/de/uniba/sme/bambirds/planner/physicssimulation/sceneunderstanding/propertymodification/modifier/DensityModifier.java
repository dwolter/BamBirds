...
public class DensityModifier implements PropertyModifier {
	final ABType objectABType;

	public DensityModifier(ABType objectABType){
	...
	@Override public BiConsumer<Simulation, Float> getFunction() {
		...
		return (sim, value) -> {
			...
			sim.getScene().getEntityPropertiesLUT().getEntityProperties(objectABType).setFixtureDensity(value);

			for(SceneEntityBase sceneEntityBase : sim.getScene().getAllEntities()) {
				if(sceneEntityBase.getAbType() == objectABType) {
					sceneEntityBase.getProperties().setFixtureDensity(value);
				}
			}
	...
	@Override public float getCurrentPropertyValue(Simulation simulation) {
		return  simulation.getScene().getEntityPropertiesLUT().getEntityProperties(objectABType).getFixtureDensity();
	}

	@Override public PropertyModifier.MODIFIER_TYPE getModifierType() {
		return MODIFIER_TYPE.DENSITY;
	}

	@Override public String toString() {
		return "Density Modifier - Type: "+ objectABType.toString();
	}
}
