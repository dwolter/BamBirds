...
public class NewPrologPlanParser extends PrologPlanParser {
	private static final Logger log = LogManager.getLogger(NewPrologPlanParser.class);
	...
	public List<Target> parsePlans(String plans) {
		...
		if (plans.isEmpty()) {
			log.error("[PPP] Got empty string, returning empty list of Targets!");
			return targets;
		}
	...
	private Target parseSinglePlan(String plan) throws IllegalArgumentException {
		...
		try {
			...
			log.error("Error parsing plan, using default values. Exception:" + e.getMessage());
		}
