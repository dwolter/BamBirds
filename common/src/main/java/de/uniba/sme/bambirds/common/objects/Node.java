...
public class Node {
	...
	public void markLostAndDecreaseConfidence(double confidenceMultiplier) {
		this.gameLost = true;
		log.info("Current node lost, propagating confidence penalty ...");
		Node n = this;
		int exponent = 1;
		while (n.parent != null) {
			...
			log.info(String.format("New confidence: %.3f for %s", newConfidence, n));
	...
	public void compareCurrentSituationWithPrevSituation() {
		...
		if (stillTheSame){
			log.info("situation still looks the same ----------------");
			...
			if (!containsChildWithSaveShot(this.savedShot)){
				...
				log.info("created lost copy of this shot");
			}
		}
	}
}