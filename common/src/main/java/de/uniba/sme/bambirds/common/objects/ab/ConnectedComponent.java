...
public class ConnectedComponent {
    ...
    public Body getBody()
    {
        if (_type == ABType.Sling.id())
            return new Rect(boundingBox(), assignType(_type));
        
        if ((_type > ABType.Sling.id() &&
            _type <= ABType.Pig.id()) ||
            _type == ABType.Trajectory.id())
    ...
    public void draw(BufferedImage canvas, boolean drawEdge, boolean drawCorners)
    {
        ...
        for (int x = 2; x < _width-2; x++)
        {
            ...
            {
                canvas.setRGB(x + _left, y + _top, Colors._drawColor[_type]);
            }
    ...
    {
    	return ABType.fromID(vision_type);
    }
