...
rf_classifier_output = os.path.join(__current_working__dir,'rf_classifier.pkl')
...
def train_rf_classifier(features_training, labels_training):
    pipeline = PMMLPipeline([("classifier", RandomForestClassifier(n_estimators=1500, random_state=73, max_depth=10))])
...
def query_rf_classifier(rfclass, features_testing):
    return rfclass.predict(features_testing)
